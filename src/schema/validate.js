  ...
  if (!ajv) {
    ...
    for (const type of ['common', 'commands/post', 'message']) {
      const schema = await fs.readJSON(
        path.join(__dirname, '..', 'schema', `${type}.json`)
      )
      schemas.push(schema)
    }
...
validate('message', {
  body: 'hi',
  type: 'message',
  meta: { t: 1541532807054, hash: 'mEg9oaTE1NDE1MzI4MDcwNTQ' },
})
validate('message', {
  body: 'hi',
  type: 'message',
  meta: {
    ...
    signed: [
      {
        publicKey: 'ZeqfmS4f5GpWcf5VRiu6weXS3SR7VzX1XDs8HEqa5bEXT',
        signature:
          'ZP8TfhVzJcuQ1qEoENewE1KZUd6Jr99w9rmykrwDxMsEFVtqWhYCpJzhmWi7phTdqd792mU1Vbpvy53VqCvyQ3t9',
      },
    ],
    route: [
      {
    ...
    hash: 'mEiBoaTE1NDE1MzMwMTAxNTR2aXRpcy1qb3NvZC1qaWdhYg',
  },
  from: 'vitis-josod-jigab',
})
